		...
		public class EditorSuitConfig
		{
			...
			HardlightCollider LookupSceneReference(int index)
			{
				...
				if (MyDefinition.ZoneHolders[index] != null)
				{
					...
					Transform objToCheck = MyDefinition.AddChildObjects ? targObj.transform.Find(lookupName) : targObj.transform;
			...
			string AddComponentForSuit()
			{
				...
				for (int i = 0; i < MyDefinition.ZoneHolders.Count; i++)
				{
					if (MyDefinition.ZoneHolders[i] != null)
					{
						...
						GameObject targetGO = MyDefinition.AddChildObjects ? MyDefinition.ZoneHolders[i].transform.Find(MyDefinition.ZoneHolders[i].name + childAppendName).gameObject : MyDefinition.ZoneHolders[i];
