...
namespace VRTK
{
    ...
    public class VRTK_ControllerActions : MonoBehaviour
    {
		...
		/// The TriggerHapticPulse/3 method calls a haptic pulse for a specified amount of time rather than just a single tick. Each pulse can be separated by providing a `pulseInterval` to pause between each haptic pulse.
		/// </summary>
		/// <param name="strength">The intensity of the rumble of the controller motor. `0.0f` to `1.0f`.</param>
		/// <param name="duration">The length of time the rumble should continue for.</param>
		/// <param name="pulseInterval">The interval to wait between each haptic pulse.</param>
		/// <param name="decayRate">A multiplier applied to the strength each pulse. Best between .2f and 3.0f</param>
		public void TriggerDecayingHapticPulse(float strength, float duration, float pulseInterval, float decayRate)
		{
			if (isActiveAndEnabled)
			{
				CancelHapticPulse();
				var hapticPulseStrength = strength;// Mathf.Clamp(strength, 0f, 1f);
				var hapticModifiers = VRTK_SDK_Bridge.GetHapticModifiers();

				hapticLoop = StartCoroutine(DecayingHapticPulse(
					duration * hapticModifiers.durationModifier, 
					hapticPulseStrength, 
					pulseInterval * hapticModifiers.intervalModifier,
					decayRate));
		...
		private IEnumerator DecayingHapticPulse(float duration, float hapticPulseStrength, float pulseInterval, float decayRate)
		{
			if (pulseInterval <= 0)
			{
				yield break;
			...
			{
				VRTK_SDK_Bridge.HapticPulseOnIndex(VRTK_DeviceFinder.GetControllerIndex(gameObject), hapticPulseStrength);
				hapticPulseStrength = Mathf.Clamp((hapticPulseStrength * decayRate), 0f, 1f);
				yield return new WaitForSeconds(pulseInterval);
				duration -= pulseInterval;
			}
		}
